...
export default function CostEstimateIndexPage() {
        ...
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 13, color: C.warmGrey, margin: 0, lineHeight: 1.6 }}>
              Select a property below to generate a cost estimate.
              The classifier and design brief must have run first.
            </p>
          </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {reports.map(r => {
                  ...
                  const hasReading = !!r.results?.classification?.architecturalReading;
                  const hasCost = !!r.results?.cost_estimate;
                  const ready = hasBrief && hasReading;

                  return (
                    <div
                      key={r.id}
                      onClick={() => router.push(`/cost-estimate/${r.id}`)}
                      style={{
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                        <div>
                          ...
                          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                            ...
                            {hasBrief && <Badge color={C.sage}>Brief ✓</Badge>}
                            {hasReading && <Badge color={C.sage}>Reading ✓</Badge>}
                            {hasCost && <Badge color={C.sage}>Cost ✓</Badge>}
                            {!ready && <Badge color={C.terracotta}>Needs brief + reading</Badge>}
                          </div>
                        </div>
                        <span style={{ fontSize: 18, color: C.warmGrey }}>→</span>
                      </div>
                    </div>
                  );
                })}
